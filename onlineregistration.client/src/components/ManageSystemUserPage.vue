<template>
  ...
  <div class="action-bar">
    ...
    <button class="add-btn" @click="openAddForm">+ Add New System User</button>
  </div>
  ...
  <div v-if="showForm" class="form-overlay" @click.self="closeForm">
    <div class="form-popup">
      ...
      <form @submit.prevent="saveUser">
        ...
        <input v-model="form.password" readonly />
...
<script setup>
  ...
  async function saveUser() {
    try {
      ...
      const res = await api.post("/admin/create-user", body); // <-- change the api url to change params

      //await this.LoadSystemUsers();
      form.password = res.data.initialPassword;

      //this.showForm = false;
      //// Updated success message to reflect password is now generated by the backend
      showDialog.value = true;
      dialogTitle.value = "Success";
      dialogMessage.value = `✅ User ${form.username} created successfully! An initial password was generated by the system. User must reset on first login.`;
  ...
  async function validateForm() {
    ...
    if (!this.form.username || this.form.username.length < 3) {
      this.formValidation.username = 'Username must be at least 3 characters long.';
      isValid = false;
    }
    return isValid;
  }
